...
const QRCodePage = () => {
  ...
  // Build human-readable plain text so ANY QR scanner shows the info
  // No internet needed — data is embedded directly in the QR code
  const buildQrText = () => {
    const lines: string[] = [];
    lines.push("🚨 EMERGENCY MEDICAL PROFILE");
    lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━");
    lines.push("");
    ...
    if (profile.dob) {
      ...
      lines.push(`📅 DOB: ${new Date(profile.dob).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })} (${age} yrs)`);
    }
    if (profile.insurance) lines.push(`🏥 Insurance: ${profile.insurance}`);
    if (profile.organDonor) lines.push("💚 Organ Donor: YES");
    ...
    profile.contacts.forEach(c => {
      lines.push(`  📱 ${c.name}${c.relationship ? ` (${c.relationship})` : ""}`);
      lines.push(`     Tel: ${c.phone}`);
    });
    
    lines.push("");
    lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━");
    lines.push("Generated by SafeScan");
    lines.push("No internet needed to view");
          ...
          <div className="relative" id="qr-code-container">
            <div className="pulse-ring p-1">
              <div className="bg-white rounded-2xl p-4 glow-red">
                <QRCodeSVG id="qr-svg" value={qrData} size={200} level="L" bgColor="#ffffff" fgColor="#000000" />
              </div>
            </div>
          </div>
