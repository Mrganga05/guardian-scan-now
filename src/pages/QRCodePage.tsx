...
import { ArrowLeft, Eye, Share2, Edit, User, Droplets, AlertOctagon, Heart, Phone, MapPin, AlertTriangle, Pill, Download } from "lucide-react";
...
const QRCodePage = () => {
  ...
  // Encode profile data as base64 in a URL hash
  // When scanned, opens a beautiful GUI emergency profile page
  // Data is embedded in the URL itself — no database lookup needed
  const buildQrUrl = () => {
    const payload = {
    ...
    return `${baseUrl}/emergency#${encoded}`;
  };
          ...
          <div className="relative" id="qr-code-container">
            <div className="pulse-ring p-1">
              <div className="bg-white rounded-2xl p-4 glow-red">
                <QRCodeSVG id="qr-svg" value={qrData} size={200} level="M" bgColor="#ffffff" fgColor="#000000" />
              </div>
            </div>
          </div>
          ...
          <Button
            onClick={() => {
              ...
              img.onload = () => {
                ...
                ctx.fillText("🚨 EMERGENCY PROFILE", 300, 440);
                ...
                if (profile.allergies?.length) {
                  ...
                  ctx.fillText(`⚠️ ${profile.allergies.join(", ")}`, 300, 555);
                }
                ctx.fillStyle = "#888888";
                ctx.font = "14px sans-serif";
                ctx.fillText("Scan QR • No internet needed", 300, 600);
                ctx.fillText("Generated by SafeScan", 300, 625);
                // Emergency contacts
                profile.contacts.forEach((c, i) => {
                  ctx.fillStyle = "#cccccc";
                  ctx.font = "16px sans-serif";
                  ctx.fillText(`📞 ${c.name}: ${c.phone}`, 300, 660 + i * 25);
                });
                // Download
